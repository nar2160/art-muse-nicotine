...
aprime_by_ppm %>%
  ...
  unnest(data) %>%
  select(-n_trials) %>%
  pivot_wider(names_from = on_smoking, values_from = c(rate_hit, rate_fa, aprime, ppm_z)) %>%
  ggplot(aes(x = ppm_z_off, y = aprime_off, xend = ppm_z_on, yend = aprime_on, color = interaction(exptCond, probe))) +
  geom_segment()
...
```{r, eval = FALSE}
models <- crossing(name = c("full", "full_random"),
                    smoking_term = c("on_smoking")) %>%
  mutate(data = map(1:n(), ~raw %>%
         ...
         terms = map(smoking_term, ~c("corResp", "probe", .x, "exptCond")),
         ranefs = case_when(name == "full_random" & smoking_term == "on_smoking" ~ paste0("(1 + (corResp + probe + exptCond + on_smoking)^2 | subj_num)"),
                            name == "full_random" & smoking_term == "ppm_z" ~ paste0("(1 + (corResp + probe + exptCond)^2 | subj_num)"),
                            TRUE ~ "(1 + corResp | subj_num)"),
...
preplots_noisy_bysubj <- posterior_preplot_bysubj(object = models$model[[2]],
                                                  newdata = models$data[[2]] %>%
                                                    select("subj_num", models$terms[[2]]),
...
```{r}
signtests <- preplots_noisy_bysubj %>%
  bind_rows(model = .,
            raw = sdt_metrics %>% select(subj_num, exptCond, probe = cue, on_smoking, aprime, rate_hit, rate_fa),
            .id = "name") %>%
  mutate(iteration = coalesce(iteration, 1L)) %>%
  pivot_longer(c(aprime, rate_hit, rate_fa), names_to = "metric_type", values_to = "value") %>%
  ...
  nest(data = -c(name, iteration, metric_type, exptCond, probe)) %>%
...
signtests %>%
  ...
  ggplot(aes(x = iteration, y = estimate, color = significance)) +
  geom_hline(yintercept = .5, linetype = 3) +
  geom_errorbar(aes(ymin = conf_lower, ymax = conf_higher), width = 0.1, alpha = 0.3) +
  geom_point(size = 0.5) +
  ...
  labs(title = "ON - OFF smoking binomial sign test p-values") +
  theme(axis.text.x = element_text(hjust = 1, angle = 45))
```
...
preplots_fixef$predicted[[2]] %>%
  ...
  pivot_longer(c(aprime, rate_hit, rate_fa), names_to = "metric_type", values_to = "value") %>%
  arrange(metric_type, iteration, probe, exptCond, on_smoking) %>%
  group_by(metric_type, iteration, exptCond, probe) %>%
  summarize(diff_smoking = diff(value)) %>%
  group_by(metric)
```

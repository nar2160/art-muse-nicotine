...
posterior_preplot_bysubj <- function (object, newdata, pred_col = "y_pred", type = c("median", "by_iter", "trialwise"), draws = 1000) {
...
aprime_by_ppm <- raw %>%
  ...
  mutate(corResp = recode(corResp, `0` = "fa", `1` = "hit"),
         on_smoking = recode(on_smoking, `0` = "off", `1` = "on"),
         exptCond = recode(exptCond, `0` = "control", `1` = "relational")) %>%
  group_by(subj_num, ppm_z, on_smoking, exptCond, probe, corResp) %>%
  summarize(rate = mean(resp)) %>%
  pivot_wider(names_from = corResp, values_from = rate, names_prefix = "rate_") %>%
  mutate(aprime = sdt_aprime(rate_hit, rate_fa)) %>%
  select(-starts_with("rate")) %>%
  pivot_wider(names_from = on_smoking, values_from = c(aprime, ppm_z)) %>%
  mutate(ppm_z_diff = ppm_z_on - ppm_z_off,
  ...
  mutate(model = map(data, ~lm(aprime_diff ~ aprime_off_c + ppm_z_diff, data = .x)))
```
...
aprime_by_ppm %>%
  ...
  geom_point() +
  facet_grid(probe ~ exptCond) +
  labs(title = "model: A' diff ~ A' off (center = 0.5) + ppm diff (z units)")
```
...
boots_by_subj <- raw %>%
  ...
  mutate(corResp = recode(corResp, `0` = "fa", `1` = "hit"),
         on_smoking = recode(on_smoking, `0` = "off", `1` = "on"),
         exptCond = recode(exptCond, `0` = "control", `1` = "relational")) %>%
  nest(trials = -c(subj_num, ppm_z, on_smoking, exptCond, probe, corResp)) %>%
  ...
  mutate(boots = furrr::future_map(trials, ~bootstraps(.x, times = 1) %>%
...
boots_by_trial <- raw %>%
  ...
  # CHANGE N BOOTSTRAPS HERE
  # resample inner level: trials (honoring condition structure)
  mutate(boots = furrr::future_map(trials, ~bootstraps(.x, times = 100) %>%
                       mutate(iteration = 1:nrow(.),
...
signtests_boot %>%
  ...
  geom_errorbar(aes(ymin = conf_lower, ymax = conf_higher), width = 0.1, alpha = 0.8) +
  geom_point(size = 0.5) +
...
aprime_by_ppm_boot <- boots_by_subj %>%
  ...
  mutate(coefs = map(data, ~lm(aprime_diff ~ aprime_off_c + ppm_z_diff, data = .x) %>%
                       broom::tidy())) %>%
  select(-data) %>%
  unnest(coefs) %>%
...
```{r, eval = FALSE}
models2 <- tibble(name = c("full", "manip_check"),
                 has_valid = list(1L, 0:1),
                 terms = list(c("corResp", "probe", "on_smoking", "exptCond"),
                              c("corResp", "valid", "probe", "on_smoking", "exptCond")),
                 ranefs = c("(1 + (corResp + probe + exptCond + on_smoking)^2 | subj_num)",
                            "(1 + corResp + valid + probe + exptCond + on_smoking | subj_num)")) %>%
  mutate(data = map(1:n(), ~raw %>%
                      # EFFECT CODE TO THE HEAVENS!!!
                      mutate(corResp = corResp - 0.5,
                             exptCond = exptCond - 0.5,
                             probe = recode(probe,
                                            art = -0.5,
                                            room = 0.5),
                             on_smoking = on_smoking - 0.5)),
         data = map2(data, has_valid, ~.x %>% filter(valid %in% .y)),
         txt_formula = map_chr(terms, ~paste(.x, collapse = " + ")),
         txt_formula = case_when(name == "full" ~ paste0("resp ~ (", txt_formula, ")^3 + ", ranefs),
                                 name == "manip_check" ~ paste0("resp ~ ", txt_formula, " + corResp:valid + ", ranefs)),
...
preplots_bysubj <- models %>%
  ...
  mutate(data = map2(data, terms, ~.x %>% select("subj_num", .y) %>% distinct()),
         predicted = map2(model, data, ~posterior_preplot_bysubj(.x, .y, pred_col = "resp_pred", type = "median")),
         predicted = map(predicted, ~.x %>%
                           ...
                           mutate(corResp = recode(corResp, `-0.5` = "fa", `0.5` = "hit"),
                                  probe = recode(probe, `-0.5` = "art", `0.5` = "room"),
...
preplots_noisy_bysubj <- posterior_preplot_bysubj(object = models$model[[1]],
                                                  newdata = models$data[[1]] %>%
                                                    select("subj_num", models$terms[[1]]),
                                                  pred_col = "resp_pred", type = "trialwise", draws = 300) %>%
  mutate_at(vars(one_of("exptCond")), ~recode(.x, `-0.5` = "control", `0.5` = "relational")) %>%
  mutate(corResp = recode(corResp, `-0.5` = "fa", `0.5` = "hit"),
         probe = recode(probe, `-0.5` = "art", `0.5` = "room"),
         on_smoking = recode(on_smoking, `-0.5` = "off", `0.5` = "on")) %>%
  group_by(iteration, subj_num, on_smoking, exptCond, probe, corResp) %>%
  summarize(resp_pred = mean(resp_pred)) %>%
  ungroup() %>%
  pivot_wider(names_from = corResp,
              values_from = resp_pred,
              names_prefix = "rate_") %>%
...
save(preplots_params, preplots_bysubj, preplots_noisy_bysubj, preplots_fixef, file = "ignore/data_R/preplots_rstanarm.rda")
```
...
```{r}
preplots_raw_bysubj <- bind_rows(model_random = preplots_bysubj$predicted[[1]],
                                 model_random_noisy = preplots_noisy_bysubj %>%
                                   nest(data = -iteration) %>%
                                   sample_n(1) %>%
                                   unnest(data),
                                 raw = sdt_metrics %>%
                                 ...
                                 .id = "name") %>%
  select(-dprime, -iteration)

preplots_raw_fixef <- preplots_fixef$predicted[[1]] %>%
...
```{r}
preplots_raw_bysubj %>%
  filter(name != "model_random_noisy") %>%
...
```{r}
preplots_raw_bysubj %>%
  filter(name %in% c("model_random", "raw")) %>%
  mutate(name = recode(name, model_random = "model")) %>%
  pivot_longer(cols = c(rate_hit, rate_fa, aprime),
               names_to = "metric_type", values_to = "value") %>%
  pivot_wider(names_from = name, values_from = value, names_prefix = "value_") %>%
  mutate(error = value_model - value_raw) %>%
  group_by(metric_type, on_smoking, exptCond, probe) %>%
  summarize(rmse = sqrt(mean(error ^ 2)),
            mae = median(abs(error))) %>%
  pivot_longer(cols = c(rmse, mae), names_to = "error_type", values_to = "error") %>%
  ggplot(aes(x = "a", y = error, color = error_type)) +
  geom_hline(yintercept = 0, linetype = 3) +
  geom_point() +
...
preplots_fixef$predicted[[2]] %>%
  select(-dprime) %>%
  pivot_longer(c(aprime, rate_hit, rate_fa), names_to = "metric_type", values_to = "value") %>%
  arrange(metric_type, iteration, probe, exptCond, on_smoking) %>%
  group_by(metric_type, iteration, exptCond, on_smoking) %>%
...
preplots_raw_fixef %>%
  ...
  group_by(metric_type, iteration, exptCond) %>%
  summarize(diff_smoking_probe = diff(diff_smoking)) %>%
  group_by(metric_type, iteration) %>%

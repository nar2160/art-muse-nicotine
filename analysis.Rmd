...
```{r}
sdt_metrics %>%
  select(subj_num, on_smoking, exptCond, probe = cue, aprime) %>%
  unite(col = trial_type, on_smoking, exptCond, probe) %>%
  ungroup() %>%
  pivot_wider(values_from = aprime, names_from = trial_type) %>%
  select(-subj_num) %>%
  cor() %>%
...
preplots_bysubj <- models %>%
  ...
  mutate(data = map2(data, terms, ~.x %>% select("subj_num", .y) %>% distinct()),
         predicted = map2(model, data, ~posterior_preplot_bysubj(.x, .y, pred_col = "resp_pred", type = "median", draws = 500)),
         predicted = map_if(predicted, name != "dummy", ~.x %>%
...
```{r}
preplots_raw_bysubj <- bind_rows(model_2x2x2 = preplots_bysubj$predicted[[1]],
                                 model_dummy = preplots_bysubj$predicted[[3]],
                                 raw = sdt_metrics %>%
...
plot_params <- function (df, model_name) {
  df %>%
    ...
    labs(title = glue::glue("visual coefficients/SEs for model: {model_name}"))
}
...
preplots_raw_bysubj %>%
  ...
  ggplot(aes(x = error_type, y = error, color = name)) +
  geom_hline(yintercept = 0, linetype = 3) +
  geom_point() +

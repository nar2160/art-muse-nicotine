...
posterior_preplot_bysubj <- function (object, newdata, pred_col = "y_pred", by_iter = FALSE, draws = 1000) {
  out <- object %>%
...
>>>>>>> ceb4ad9f4f1296599ca23c9da6448f5ebf3e3a40
```
...
```{r, eval = FALSE}
models2 <- crossing(name = c("full", "full_random"),
...
preplots_fixef$predicted[[2]] %>%
  ...
  labs(x = "on or off smoking?",
       ...
       title = "room/art smoking diffs by condition",
...
```{r}
x <- preplots_bysubj$predicted[[2]] %>%
  select(subj_num:exptCond, aprime, rate_hit, rate_fa) %>%
  bind_rows(model = .,
  ...
  nest(data = -c(name, metric_type, exptCond, probe)) %>%
  ...
  ggplot(aes(x = interaction(exptCond, probe), y = p_value, color = name)) +

function artmusePatient_practice (subjectNumber, subjectName)
%
% function artmusePatient_practice(subjectNumber, subjectName, startRun, endRun, fMRI, eyeTrack)
%
...
% 5 trials of each state in blocked form (10 trials total); control and experimental trials intermixed
%
...
% using
%
% instructions were changed to state 'press the 1 key for a match and the 2
% key for a non-match'
%
% be sure to have this code in the same directory as the 'practiceStimuli'
% folder
%
...
% 'ffind . -name '*DS_Store' -type f -delete'
%
...
%% some parameters
    ...
    % trial numbers
        numExptTrials = 5; % total experimental trials
        numControlTrials = 5; % total control trials
        totalNumTrials = numExptTrials + numControlTrials;
        stimNum = 1:totalNumTrials; % should have this number unique base image trials in practice stimuli directory
        
        propValid = .8; % proportion of valid trials
        
        % only one invalid trial for experimental and one for control (thus, below doesn't apply)
...
%% keyboard
 ...
 %% set up screens
  ...
  %% set up conditions and randomize, separately for art and room blocks
        ...
        % conditions: control trials (0) or experimental trials(1)
            exptCond.art = [0; 0; 0; 1; 1];
            exptCond.room = [0; 0; 1; 1; 1];
 
        % cue type (art or room)
            cueType.art = repmat(1, totalNumTrials/2, 1);
            cueType.room = repmat(2, totalNumTrials/2, 1);
            
        % probe type (art or room)
            probeType.art = [2; 1; 1; 1; 1;];
            probeType.room = [2; 2; 2; 2; 1];
        ...
        % show art or room match? probed match shown 50% of the time
            ...
            % on invalid trials, other (probed) match is shown 50% of the time; on remaining 50% of invalid trials, cued match is shown half the time, neither match shown half the time
                ...
                cuedMatchShown.art = [0; 0; 1; 0; 1];
                cuedMatchShown.room = [0; 1; 0; 1; 1]; 

                otherMatchShown.art = [1; 0; 0; 1; 0];
                otherMatchShown.room = [1; 0; 0; 0; 0];
 ...
 %% save setup variables in 'stim' structure 
    ...
    filename = strcat('artmusePatient_practice_', num2str(subjectNumber), '_', subjectName); 
...
%% open and set up files for data collection / on-line monitoring
    ...
    % open and set up files for data collection
        dataFile = fopen(['artmusePatient_practice_' num2str(subjectNumber) '_' subjectName '.txt'],'a'); 
        fprintf(dataFile,'************************************************\n'); 
    ...
    % print to command window to monitor real-time performance (in dual-screen mode)
        fprintf('************************************************\n'); 
        fprintf('* artmuse patient study - practice\n'); 
